...
void displayHelpMessage() {
    ...
    std::cout << "\033[38;5;51m▏To save time, you can substitute \"cb\" for \"clipboard.\"\033[0m" << std::endl;
...
void setupAction() {
    if (items.at(0) == "cut" || items.at(0) == "./cut") {
        action = Action::Cut;
    } else if (items.at(0) == "copy" || items.at(0) == "./copy") {
        action = Action::Copy;
    } else if (items.at(0) == "paste" || items.at(0) == "./paste") {
        action = Action::Paste;
    } else if (items.at(0) == "clipboard" || items.at(0) == "cb" || items.at(0) == "./clipboard" || items.at(0) == "./cb") {
        if (items.size() > 1) {
            if (items.at(1) == "cut") {
                action = Action::Cut;
                ...
                std::cout << "\033[38;5;196m╳ You did not specify a valid action. \033[38;5;219mTry using \033[1mcut, copy, or paste\033[0m\033[38;5;219m instead, like \033[1mclipboard copy\033[0m" << std::endl;
                exit(1);
            }
            items.erase(items.begin());
        } else {
            std::cout << "\033[38;5;196m╳ You did not specify an action. \033[38;5;219mTry adding \033[1mcut, copy, or paste\033[0m\033[38;5;219m to the end, like \033[1mclipboard copy\033[0m" << std::endl;
            exit(1);
        }
    } else {
        throw std::runtime_error("Could not determine action");
    }
    items.erase(items.begin());
}
...
void checkForItemExistence() {
    for (const auto& i : items) {
        if (!fs::exists(i)) {
            std::cout << "\033[38;5;196m╳ Your item \033[38;5;219m" << i << "\033[38;5;196m does not exist.\033[0m" << std::endl;
            exit(1);
        }
    }
}
...
void showFailures() {
    if ((files_failed == 1) != (directories_failed == 1)) {
        if (action == Action::Copy) {
            ...
            std::cout << "\033[38;5;196m╳\033[91m Failed to paste\033[0m" << std::endl;
        }
    } else {
