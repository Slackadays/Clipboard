...
Introducing the Clipboard Project, or CB for short. It all started when I needed something way more powerful than those other tools like `xclip`, `wl-clipboard`, and `pbcopy`. I began by writing a few scripts and experimenting with what might be possible. Unfortunately, those scripts just weren't cutting it. A few revisions and then a complete rewrite later, and suddenly I had created the Clipboard Project!
